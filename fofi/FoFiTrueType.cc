...
std::unique_ptr<FoFiTrueType> FoFiTrueType::make(std::vector<unsigned char> &&fileA, int faceIndexA)
{
    auto ff = std::make_unique<FoFiTrueType>(std::move(fileA), faceIndexA);
    if (!ff->parsedOk) {
        return nullptr;
    }
    return ff;
