...
void QPainterOutputDev::updateFont(GfxState *state)
{
    ...
    if (codeToGIDIt != m_codeToGIDCache.end()) {
        ...
        case fontTrueType:
        case fontTrueTypeOT: {
            auto ff = (fontLoc->locType != gfxFontLocEmbedded) ? FoFiTrueType::load(fontLoc->path.c_str(), fontLoc->fontNum) : FoFiTrueType::make(std::move(fontBuffer).value(), fontLoc->fontNum);
        ...
        case fontCIDType0C: {
            ...
            if (!m_useCIDs) {
                auto ff = (fontLoc->locType != gfxFontLocEmbedded) ? std::unique_ptr<FoFiType1C>(FoFiType1C::load(fontLoc->path.c_str())) : std::unique_ptr<FoFiType1C>(FoFiType1C::make(std::move(fontBuffer).value()));
