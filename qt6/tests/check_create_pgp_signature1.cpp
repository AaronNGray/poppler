...
#endif
        qputenv("GNUPGHOME", TESTDATADIR "unittestcases/check_create_pgp_signature1_keyring/");
    }
...
void TestSignWithGnupgPgp::testPgpSignVerify()
{
    ...
    auto signingResult = doc->sign(std::string { d.filePath("signedFile.pdf").toStdString() }, std::string { "36E39802E4F49A259091DA69381B80FEF3535BC1" }, std::string {}, std::make_unique<GooString>("newSignatureFieldName"), /*page*/ 1,
    ...
    if (activeBackendType == CryptoSign::Backend::Type::NSS3) {
        ...
        auto signedDoc = std::make_unique<PDFDoc>(std::make_unique<GooString>(d.filePath("signedFile.pdf").toStdString()));
